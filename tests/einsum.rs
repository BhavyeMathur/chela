...
test_for_common_numeric_dtypes!(
    test_einsum_sums, {
        for n in 1..17 {
            ...
            let expected = a.sum();
            let result = redstone_ml::einsum([&a], (["i"], ""));
            assert_almost_eq!(result, expected);

            // trace
            let a = NdArray::arange(0, n * n).astype::<T>();
            let a = a.reshape([n, n]);
            let result = redstone_ml::einsum([&a], (["ii"], ""));
...
test_for_common_numeric_dtypes!(
    test_einsum_sum_slice, {
        ...
        for n in (1..50).step_by(5) {
            ...
            let expected = a.sum();
            let result = redstone_ml::einsum([&a], (["ij"], ""));
            assert_almost_eq!(result, expected);

            let expected = a.sum_along(Axis(0));
            let result = redstone_ml::einsum([&a], (["ij"], "j"));
            assert_almost_eq!(result, expected);

            let expected = a.sum_along(Axis(1));
            let result = redstone_ml::einsum([&a], (["ij"], "i"));
            assert_almost_eq!(result, expected);
        }
        ...
        for n in (3..24).step_by(5) {
            ...
            let expected = a.sum();
            let result = redstone_ml::einsum([&a], (["ijk"], ""));
            assert_almost_eq!(result, expected);

            let expected = a.sum_along(Axis(0));
            let result = redstone_ml::einsum([&a], (["ijk"], "jk"));
            assert_almost_eq!(result, expected);

            let expected = a.sum_along(Axis(1));
            let result = redstone_ml::einsum([&a], (["ijk"], "ik"));
            assert_almost_eq!(result, expected);

            let expected = a.sum_along(Axis(2));
            let result = redstone_ml::einsum([&a], (["ijk"], "ij"));
            assert_almost_eq!(result, expected);

            let expected = a.sum_along(Axis(2));
            let expected = expected.sum_along(Axis(1));
            let result = redstone_ml::einsum([&a], (["ijk"], "i"));
            assert_almost_eq!(result, expected);

            let expected = a.sum_along(Axis(2));
            let expected = expected.sum_along(Axis(0));
            let result = redstone_ml::einsum([&a], (["ijk"], "j"));
            assert_almost_eq!(result, expected);

            let expected = a.sum_along(Axis(1));
            let expected = expected.sum_along(Axis(0));
            let result = redstone_ml::einsum([&a], (["ijk"], "k"));
            assert_almost_eq!(result, expected);
        }
    }
);
...
test_for_common_numeric_dtypes!(
    test_einsum_matmul, {
        ...
        let result = redstone_ml::einsum([&a, &b], (["ij", "jk"], "ik"));
        assert_almost_eq!(result, expected);
...
test_for_common_numeric_dtypes!(
    test_einsum_matrix_vector, {
        ...
        let result = redstone_ml::einsum(&[&a, &b], (&["ij", "j"], "i"));
        assert_almost_eq!(result, expected);
    }
);
...
test_for_all_numeric_dtypes!(
    test_einsum_pointwise_multiplication, {
        ...
        let result = redstone_ml::einsum([&a, &b], (&["ij", "ij"], "ij"));
        assert_almost_eq!(result, expected);
...
test_for_common_numeric_dtypes!(
    test_einsum_ij_ki_j, {
        ...
        let result = redstone_ml::einsum(&[&a, &b], (["ij", "ki"], "j"));
        assert_almost_eq!(result, expected);
    }
);
...
test_for_common_numeric_dtypes!(
    test_einsum_ij_ki_i, {
        ...
        let result = redstone_ml::einsum(&[&a, &b], (["ij", "ki"], "i"));
        assert_almost_eq!(result, expected);
    }
);
...
test_for_all_numeric_dtypes!(
    test_einsum_sum_along_axes, {
        ...
        let result = redstone_ml::einsum([&a, &b], (["ij", "jk"], "i"));
        assert_almost_eq!(result, expected);

        let expected = NdArray::new([11, 90]).astype::<T>();
        let result = redstone_ml::einsum([&a, &b], (["ij", "jk"], "j"));
        assert_almost_eq!(result, expected);
...
test_for_all_numeric_dtypes!(
    test_einsum_sum_product, {
        ...
        let result = redstone_ml::einsum([&a, &b], (["ij", "ik"], ""));
        assert_almost_eq!(result, expected);

        let expected = NdArray::scalar(101).astype::<T>();
        let result = redstone_ml::einsum([&a, &b], (["ij", "jk"], ""));
        assert_almost_eq!(result, expected);

        let expected = NdArray::scalar(71).astype::<T>();
        let result = redstone_ml::einsum([&a, &b], (["ij", "ki"], ""));
        assert_almost_eq!(result, expected);

        let expected = NdArray::scalar(93).astype::<T>();
        let result = redstone_ml::einsum([&a, &b], (["ij", "kj"], ""));
        assert_almost_eq!(result, expected);
    }
);
...
test_for_all_numeric_dtypes!(
    test_einsum_2operands_to_3d, {
        ...
        let result = redstone_ml::einsum([&a, &b], (["ij", "kj"], "ijk"));
        assert_almost_eq!(result, expected);

        let expected = NdArray::new([[[5, 6], [10, 12]], [[0, 0], [10, 20]]]).astype::<T>();
        let result = redstone_ml::einsum([&a, &b], (["ij", "ik"], "ijk"));
        assert_almost_eq!(result, expected);
    }
);
...
test_for_all_numeric_dtypes!(
    test_einsum_matrix_outer_product, {
        ...
        let result = redstone_ml::einsum([&a, &b], (["ij", "kl"], "ijkl"));
        assert_almost_eq!(result, expected);
    }
);
